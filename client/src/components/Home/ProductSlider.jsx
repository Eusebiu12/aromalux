    ...
    <div ref={scrollRef} className="flex space-x-6 overflow-x-auto scrollbar-hide pb-4">
      {
        products.map((product) => {
          return(
            <Link key={product.id} to={`/product/${product.id}`} className="flex-shrink-0 w-80 glass-card
              ...
              <div className = "relative overflow-hidden rounded-lg mb-4">
                ...
                <img src={
                 ...
                 ? product.images[0].url
                  ...
                  alt={product?.name} className="w-full h-48 object-contain
                ...
                <div className="absolute top-3 left-3 flex flex-col space-y-2">
                  {
                    ...
                    )}
                    {
                      ...
                      font-semibold rounded">TOP RATED</span>
                      )
                    }
                 </div>
                 {/* QUICK Add TO CART*/}
                 <button onClick={(e) => handleAddToCart(product,e)} className="absolute bottom-3 right-3
                 p-2 glass-card  hover:glow-on-hover animate-smooth opacity-0 group-hover:opacity-100 transition-opacity"
              ...
              <div>
                ...
                <div>
                  <span className={`text-xs px-2 py-1 rounded ${
                    product.stock > 5 ? "bg-green-500/20 text-gray-400" : product.stock > 0 
