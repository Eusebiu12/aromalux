    ...
    <div className="p-6">
      {
        cart && cart.length === 0 ? (
          <div className = "text-center py-12">
            <p className = "teext-muted-foreground">Your cart is empty. </p>
          ...
          <div className="space-y-4 mb-6">
            {
              cart && cart.map(item => {
                return(
                  <div key={item.product.id} className="glass-cad p-4">
                    <div className="flex items-start space-x-4">
                      <img src={item.product?.images?.[0]?.url || "/candela_home_center.jpg"} alt={item.product.name}
                       className="w-16 h-16 object-cover rounded-lg"/>
                       <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-foreground truncate">
                          {item.product.name}
                        </h3>
                        <p className="text-primary font-semibold"> RON {item.product.price}
                        </p>
