    ...
    authUser && (
      <form onSubmit={handleReviewSubmit} className="mb-8 space-y-4">
        ...
        <div className="flex items-center space-x-2">
          {
            [...Array(5)].map((_, i)=>{
              return(
                <button key={i} type="button" onClick={()=> setRating(i+1)} className={` text-2xl ${i < rating ? "text-yellow-400" 
                  : "text-gray-300"
                }`}>
                  ✩
                </button>
        ...
        <textarea value={comment} onChange={(e)=> setComment(e.target.value)}
           ...
           <button type="submit" disabled={isPostingReview} className="px-6 py-2 rounded-lg bg-primary text-primary-foreground font-semibold hover:glow-on-hover animate-smooth disabled:opacity-50"
           >
  ...
  <h3 className="text-xl font-semibold text-foreground mb-6">Customer Review</h3>
  {
    productReviews && productReviews.length > 0 ? (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-4 mb-2">
                      ...
                      <p className="text-muted-foreground mb-2">
                        {review.comment}</p>
                        {
                          (authUser?.id === review.reviewer?.id || authUser?.role === "Admin") && (
                            <button onClick={() =>
      ...
      <p className="text-muted-foreground">
        No reviews yes.Be the first one to review this product.</p>
    )
  }
  </>
  );
