      ...
      <div className="min-h-screen pt-20 flex items-center justify-center">
        <div className="text-center glass-panel max-w-md">
          ...
          <p className="text-muted-foreground mb-8">
            Looks like you have not added any items to your shopping cart yet.</p>
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
            {cartItemsCount} item{cartItemsCount !== 1 ? "s" : ""} in your cart</p>
      </div>
              ...
              return(
                <div key={item.product.id} className="glass-card p-6">
                  <div className="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-6">
                    ...
                    <img src={item.product.images[0].url} alt={item.product.name} className="w-24 h-24 object-contain rounded-lg hover:scale-105 transition-transform"
                    />
                    </Link>

                    <div className="flex-1 min-w-0">
                      <Link to={`/product/${item.product.id}`} className="block hover:text-primary transition-colors">
                      <h3 className="text-lg font-semibold text-foreground mb-1">{item.product.name}
                        </h3>
                        </Link>
                        ...
                        <div className="flex items-center space-x-2">
                          <span className="text-xl font-bold text-priamry">
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="w-12 text-center font-sembold text-lg">{item.quantity}</span>
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-green-500">
                  {total >=120 ? "Free" : "30 RON"}
                   </span>
              </div>
              <div className="border-t border-[hsla(var(--glass-border))] pt-4">
                <div className="flex justify-between">
                  <span className="text-lg font-semibold">Total</span>
                  <span>{total >=120 ? (total).toFixed(2) : (total+30).toFixed(2)}</span>
                </div>
              </div>
            </div>
            {
              authUser && (
                <Link to={"/payment"} className="w-full block text-center py-4 gradient-primary
                 text-primary-foreground rounded-lg hover:glow-on-hover
                  animate-smooth font-semibold mb-4"
                  >
                    Proceed to Checkout</Link>
              )
            }
            <Link to={"/products"} className="w-full block text-center py-4
             bg-secondary text-foreground gradient-primary rounded-lg 
              animate-smooth font-semibold mb-4 border-border hover:bg-accent"
                  >
