...
export const dashboardStats = catchAsyncErrors(async (req, res, next) => {
  ...
  const lowStockProductsQuery = await database.query(`
        SELECT name, stock FROM products WHERE stock > 0 AND stock <= 5
      `);

  //  2. Products with stock equal to 0 (OUT OF STOCK)
  const lowStockProducts = lowStockProductsQuery.rows;

  const outOfStockQuery = await database.query(`
     SELECT COUNT(*) AS count FROM products WHERE stock = 0
      `);
  const outOfStockCount = parseInt(outOfStockQuery.rows[0].count) || 0;
